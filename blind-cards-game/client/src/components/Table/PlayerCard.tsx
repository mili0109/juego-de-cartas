...
export const PlayerCard: React.FC<Props> = ({ player, isCurrentTurn, isSelf, isSelectable, onSelect }) => {
  ...
  return (
    <button
      ...
      } ${isSelectable ? 'hover:bg-slate-700 cursor-pointer' : ''} bg-slate-800`}
    >
      <div
        className={`w-16 h-24 rounded-lg flex items-center justify-center text-2xl font-bold border-2 ${
          hidden
            ? 'bg-gradient-to-br from-indigo-700 to-indigo-900 border-indigo-500'
            : 'bg-white text-slate-900 border-slate-300'
        }`}
      >
        {hidden ? '🂠' : cardLabel(player.card)}
      </div>
