...
- Al elegir el gamertag, cada jugador también elige un **avatar** (emoji) de una grilla — queda guardado en `localStorage` junto al nombre.
- Al crear la sala, el anfitrión define cuántas **vidas** tiene cada jugador para adivinar su carta (de 3 a 9). Cada vez que alguien **no** adivina, pierde una vida; al llegar a 0 queda **eliminado** (no puede seguir jugando, pero se lo sigue viendo en la mesa, atenuado). La partida termina cuando se agota el mazo, alguien llega al puntaje de victoria, o queda un solo jugador en pie.
- Al perder una vida, la carta de ese jugador tiembla brevemente y suena un efecto (ver sección de sonido más abajo).

## Sonido al perder una vida

El código ya dispara un sonido cada vez que alguien pierde una vida (`client/src/services/sound.ts`), pero **no incluye ningún archivo de audio con copyright** — no se puede distribuir la banda sonora de un anime (ni de Re:Zero ni de cualquier otro) dentro de un proyecto. Para personalizarlo:

1. Conseguí un archivo de audio corto (`.mp3`) del que tengas los derechos (grabación propia, banco de sonidos libres, etc.).
2. Guardalo como `client/public/sounds/life-lost.mp3`.
3. Listo — se reproduce automáticamente. Mientras ese archivo no exista, se escucha un sonido sintetizado genérico como reemplazo temporal.

## Paleta de colores

El fondo general usa el degradado `linear-gradient(179deg, #4000F5, #000000)` (violeta a negro), con los paneles de la interfaz en un tono translúcido oscuro sobre ese fondo (`client/src/index.css`).
...
- Las cartas tienen una animación sutil de aparición al repartirse, un leve flotado continuo, un pulso en la carta del jugador cuyo turno está activo, y una sacudida al perder una vida.
