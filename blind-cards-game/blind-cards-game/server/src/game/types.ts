...
export interface Player {
  id: string; // socket.id
  gamertag: string;
  avatar: string; // emoji de avatar elegido
  isHost: boolean;
  card: Card | null; // carta real en el servidor (nunca se envía al dueño)
  guessedCount: number;
  lives: number; // vidas restantes para adivinar su propia carta
  eliminated: boolean; // true cuando se queda sin vidas
  connected: boolean;
}
...
export interface RoomConfig {
  ...
  livesPerPlayer: number; // 3-9, vidas iniciales de cada jugador
}
